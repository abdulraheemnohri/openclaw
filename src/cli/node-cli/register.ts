...
export function registerNodeCli(program: Command) {
  ...
  node
    .command("install")
    .description("Install the node host service (launchd/systemd/schtasks/termux-services)")
    .option("--host <host>", "Gateway host")
    .option("--port <port>", "Gateway port")
    .option("--tls", "Use TLS for the gateway connection", false)
    .option("--tls-fingerprint <sha256>", "Expected TLS certificate fingerprint (sha256)")
  ...
  node
    .command("uninstall")
    .description("Uninstall the node host service (launchd/systemd/schtasks/termux-services)")
    .option("--json", "Output JSON", false)
    .action(async (opts) => {
  ...
  node
    .command("stop")
    .description("Stop the node host service (launchd/systemd/schtasks/termux-services)")
    .option("--json", "Output JSON", false)
    .action(async (opts) => {
  ...
  node
    .command("restart")
    .description("Restart the node host service (launchd/systemd/schtasks/termux-services)")
    .option("--json", "Output JSON", false)
    .action(async (opts) => {
